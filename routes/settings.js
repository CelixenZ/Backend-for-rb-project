...
router.get('/', protect, requireModulePermission('settings', 'canRead'), asyncHandler(async (req, res) => {
...
router.post('/', protect, requireModulePermission('settings', 'canCreate'), asyncHandler(async (req, res) => {
  const {
    companyName,
    companyNameKh,
    address,
  ...
  if (!companyName || !address || !phone || !email) {
    throw new ApiError(400, 'Company name, address, phone, and email are required.');
  }
  ...
  return res.status(201).json({ message: 'Company settings saved successfully.', settings });
}));

router.put('/', protect, requireModulePermission('settings', 'canWrite'), asyncHandler(async (req, res) => {
  const existing = await prisma.companySettings.findFirst();

  if (!existing) {
    throw new ApiError(404, 'Company settings must be created before they can be updated.');
  }

  const updateData = {
    companyName: req.body.companyName !== undefined ? req.body.companyName : existing.companyName,
    companyNameKh: req.body.companyNameKh !== undefined ? req.body.companyNameKh : existing.companyNameKh,
    ...
    defaultTaxRate: req.body.defaultTaxRate !== undefined ? req.body.defaultTaxRate : existing.defaultTaxRate,
    currency: req.body.currency !== undefined ? req.body.currency : existing.currency,
    contractPrefix: req.body.contractPrefix !== undefined ? req.body.contractPrefix : existing.contractPrefix,
    logoUrl: req.body.logoUrl !== undefined ? req.body.logoUrl : existing.logoUrl
  };

  const updated = await prisma.companySettings.update({ where: { id: existing.id }, data: updateData });
