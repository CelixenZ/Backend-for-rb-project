...
const { getAllAlertByContractIdForUser } = require("../repository/contractAlertRepo");
...
router.post("/create-many", protect, async (req, res) => {
    if (req.user.role && req.user.role !== "ADMIN") {
        return res.status(403).json("Access Denied");
    }
...
router.get("/id/:contractId", protect, async (req, res) => {
    if (req.user.role && req.user.role !== "ADMIN") {
        return res.status(403).json("Access Denied");
    }
...
router.put("/:contractId", protect, async (req, res) => {
    if (req.user.role && req.user.role !== "ADMIN") {
        return res.status(403).json("Access Denied");
    }
...
router.get("/push", protect, async (req, res) => {
    if (req.user.role && req.user.role !== "ADMIN") {
        return res.status(403).json("Access Denied");
    }
