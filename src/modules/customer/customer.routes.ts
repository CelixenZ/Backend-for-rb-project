...
router.get(
  "/",
  protect,
  requireModulePermission("customers", "canRead"),
...
router.post(
  "/",
  protect,
  requireModulePermission("customers", "canCreate"),
  asyncHandler(async (req, res) => {
    const {
      nameEn,
      nameKh,
    ...
    if (!nameEn || !nameKh || !representative || !phone) {
      throw new ApiError(
        400,
        "Required fields: nameEn, nameKh, representative, phone.",
      );
    }
...
router.put(
  "/:id",
  protect,
  requireModulePermission("customers", "canWrite"),
  asyncHandler(async (req, res) => {
    const { id } = req.params;
    const {
      nameEn,
...
router.delete(
  "/:id",
  protect,
  requireModulePermission("customers", "canDelete"),
  asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    if (req.user.role === "STAFF" && customer.userId !== req.user.id) {
      throw new ApiError(
        403,
        "Forbidden. You can only delete your own customer records.",
      );
    }
