...
router.get(
  "/",
  protect,
  requireModulePermission("suppliers", "canRead"),
...
router.post(
  "/",
  protect,
  requireModulePermission("suppliers", "canCreate"),
  asyncHandler(async (req, res) => {
    const {
      nameEn,
      nameKh,
...
router.put(
  "/:id",
  protect,
  requireModulePermission("suppliers", "canWrite"),
  asyncHandler(async (req, res) => {
    ...
    if (req.user.role === "STAFF" && supplier.userId !== req.user.id) {
      throw new ApiError(
        403,
        "Forbidden. You can only update your own supplier records.",
      );
    }
    ...
    const updateData = {
      ...
      addressEn:
        req.body.addressEn !== undefined
          ? req.body.addressEn
          : supplier.addressEn,
      addressKh:
        req.body.addressKh !== undefined
          ? req.body.addressKh
          : supplier.addressKh,
...
router.delete(
  "/:id",
  protect,
  requireModulePermission("suppliers", "canDelete"),
  asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    const supplier = await prisma.supplier.findUnique({ where: { id } });
    ...
    if (req.user.role === "STAFF" && supplier.userId !== req.user.id) {
      throw new ApiError(
        403,
        "Forbidden. You can only delete your own supplier records.",
      );
    }
