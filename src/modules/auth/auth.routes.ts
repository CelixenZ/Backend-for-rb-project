...
const requireJwtSecrets = () => {
  if (!env.JWT_ACCESS_SECRET) {
    console.error(
      "❌ CRITICAL ERROR: JWT_SECRET is not defined in your environment variables!",
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { tokens: newTokens, user: updatedUser } = await refreshUserSession(
      user.id,
    );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    return res.json({
      message:
        "Password updated successfully. All sessions have been invalidated. Please log in again.",
    });
  }),
);
