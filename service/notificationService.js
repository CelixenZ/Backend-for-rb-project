const { ContractAlertLogStatus, NotificationChannel } = require("@prisma/client");
const prisma = require("../lib/prisma");
const { convertToKhTime } = require("../utils/timezoneUtil");
const mailSender = require("./mailSender");
const contractAlertRepo = require("../repository/contractAlertRepo");
const ApiError = require("../utils/apiError");
const { isUserOnline } = require("../socket/util");
...
const createAlert = async ({ contractId, userId, dayBeforeExpiry }) => {
    ...
    if (existingContract) {
        ...
        const daysDifferent = (expiredDateUTC - currentDateUTC) / (1000 * 60 * 60 * 24);
        if (daysDifferent < dayBeforeExpiry) {
            throw new ApiError(400, "Contract will be expired before reminding alert");
        }
        ...
        const alertDateTimestamp = expiredDateUTC.getTime() - (dayBeforeExpiry * 1000 * 60 * 60 * 24);
...
const createManyAlert = async ({ contractId, userId, daysBeforeExpiry }) => {
    ...
    if (daysBeforeExpiry.includes(0)) throw new ApiError(400, "DaysBeforeExpiry can not be 0")

    const s = new Set(daysBeforeExpiry);
    if (s.size !== daysBeforeExpiry.length) throw new ApiError(400, "DaysBeforeExpiry can not contain duplicate value");
    ...
    for (const i of daysBeforeExpiry) {
        if (daysDifferent < i) {
            throw new ApiError(400, "One or more Contract will be expired before reminding alert");
        }
        condition.push({
            contractId: contractId, 
            userId: userId, 
...
const editManyAlerts = async ({ userId, contractId, daysBeforeExpiry }) => {
    ...
    const alerts = await contractAlertRepo.getAllAlertByContractIdForUser(userId, contractId);
...
const notifyUsers = async (io) => {
    ...
    for (const i of activeAlert) {
        const daysDifferent = Math.floor((i.contract.endDate - nowUTC) / (1000 * 60 * 60 * 24));
        ...
        if (daysDifferent == i.dayBeforeExpiry) {
            ...
            const alertDateTimestamp = i.contract.endDate.getTime() - (i.dayBeforeExpiry * 1000 * 60 * 60 * 24);
            const alertDateUTC = new Date(alertDateTimestamp);
            ...
            completedAlert.push({
                ...
                status: sentEmail ? ContractAlertLogStatus.SENT : ContractAlertLogStatus.FAILED
            });
            ...
            completedAlert.push({
                ...
                alertDate: alertDateUTC,
                status: pushed ? ContractAlertLogStatus.SENT : ContractAlertLogStatus.FAILED
            });
        }
    }
    ...
    if (completedAlert.length > 0) {
        ...
        await contractAlertRepo.disableAlertsByIDs(completedAlert.map(i => i.ceaId));
    }
}

const notifyUserByEmail = async ({ email, contractId, contractTitle, daysBeforeExpiry, endDate }) => {
    try {
...
const notifyUserByPush = async ({ io, userId, contractId, contractTitle, daysBeforeExpiry, endDate }) => {
    try {
...
module.exports = {
    ...
    createManyAlert: createManyAlert,
    editManyAlerts: editManyAlerts,
    countUnreadNotificationsForUser: countUnreadNotificationsForUser,
    getAllPushedNofiticationsForUser: getAllPushedNofiticationsForUser
}